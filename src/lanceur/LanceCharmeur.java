...
import client.StrategieCharmeur;
...
public class LanceCharmeur {
	private static String usage = "USAGE : java " + LanceCharmeur.class.getName() + " [ port [ ipArene ] ]";
	...
	public static void main(String[] args) {
		...
		try {
			logger = new LoggerProjet(true, "charmeur_" + nom + groupe);
		} catch (IOException e) {
			e.printStackTrace();
		...
		try {
			...
			logger.info("Lanceur", "Creation du charmeur...");
			...
			new StrategieCharmeur(ipArene, port, ipConsole, nom, groupe, caracts, nbTours, position, logger);
			logger.info("Lanceur", "Creation du charmeur reussie");
