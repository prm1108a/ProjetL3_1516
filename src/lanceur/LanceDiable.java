...
public class LanceDiable {
	private static String usage = "USAGE : java " + LanceDiable.class.getName() + " [ port [ ipArene ] ]";
	...
	public static void main(String[] args) {
		...
		try {
			logger = new LoggerProjet(true, "diable_" + nom + groupe);
		} catch (IOException e) {
			e.printStackTrace();
		...
		try {
			...
			logger.info("Lanceur", "Creation du diable...");
			...
			new StrategieDiable(ipArene, port, ipConsole, nom, groupe, caracts, nbTours, position, logger);
			logger.info("Lanceur", "Creation du diable reussie");
